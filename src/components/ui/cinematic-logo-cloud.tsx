...
export function CinematicLogoCloud({
      ...
      >
        ...
        <Marquee speed={35}>
          {clients.map((brand) => (
            <div
              key={brand.name}
              className="flex shrink-0 items-center justify-center rounded-xl border border-zinc-200/80 bg-white px-5 py-3 shadow-sm dark:border-white/8 dark:bg-zinc-900 mx-2"
            >
  ...
  return (
    <div
      className={cn(
        "w-full bg-white dark:bg-slate-900 py-16",
        className,
      )}
    >
      <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-foreground dark:text-white mb-2 tracking-wide">
          {eyebrow}
        </h2>
        {description && (
